...
class OracleGame:
    ...
    def check_victory(self) -> bool:
        ...
        # Victory: all cards are in their correct groups
        for group_num in range(1, 14):
            for card in self.groups[group_num]:
    ...
    def check_defeat(self) -> Optional[str]:
        ...
        if not self.groups[self.current_group]:
            return f"Grupo {self.current_group} vacío"
        ...
        if (current_card.value == self.current_group and 
            ...
            return f"Auto-loop: carta {current_card.rank} apunta al mismo grupo {self.current_group}"
